...
document.querySelectorAll(".btn").forEach(e => {
    e.addEventListener("click", function () {
        ...
        if(countClick>=4){
            socket.emit("process", { value: value, move: e.id, name: name, idGame: currentId });
        }else{
...
socket.on("playing", (e) => {
    ...
    if (p2id != '') {
        ...
        document.getElementById(`${p2id}`).style.color = "blue"
    }
...
socket.on("sudden-death",(e)=>{
    ...
    if(foundObject.sum % 2 == 0){
        if (p2id != '') {
            document.getElementById(`${p2id}`).style.color = "rgba(0, 0, 0, 0.5)";
            //socket.emit("deleteMove", {move: p2id})
        }
    }else{
        if (p1id != '') {
            document.getElementById(`${p1id}`).style.color = "rgba(255, 0, 0, 0.5)";
            //socket.emit("deleteMove", {move: p1id})
        }
    }
    socket.emit("deleteMove", {game_id: foundObject.id})
})

socket.on("deleteMove",(e)=>{
    const partida = e.game;
    const moveToDelete = e.buttons[1];
    console.log(`boton para borrar: ${moveToDelete}`)
    document.getElementById(`${moveToDelete}`).innerText="";
    document.getElementById(`${moveToDelete}`).disabled=false;

    socket.emit("moveDeleted", {game_id: partida.id})
})
